...
http://cpp-school.unigine.com/?utm_source=habrahabr&utm_medium=post&utm_campaign=cpp_school_1_1
...
Программа должна компилироваться под MSVC 2013.
(Автопроверка (компилируемости) использует именно его. То есть если вам доступен только Borland C++ 3.1, вот можно сделать несколько попыток и скомпилироваться с её помощью.)

Можно использовать язык в рамках стандарта, те. C++ и ещё STL.
Loki, Boost, Qt и т.п. сторонние библиотеки использовать нельзя (да и не выйдет).
Самопальные контейнеры в виде сторонних библиотек тоже использовать нельзя (да и не выйдет).
Иначе можно. В целом, можно всё, что влезает в 1 файл и компилируется голым VS 2013.
